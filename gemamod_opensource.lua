 ...
 ["!addtime"] =
  {
    ...
    function (cn, args)
      ...
      settimeleft(tonumber(args[1]))
       say(string.format("\fR [SERVER INFO] \f4 Time remaining changed to %d", args[1]), cn)
        else
        say(string.format("\f3 ERROR: Time has to be between 3 and 15 minutes, %s!", getname(cn)), cn)
end
    end
  };
